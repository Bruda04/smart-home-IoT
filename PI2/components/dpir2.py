...
def publisher_task(event, dpir_batch, hostname='localhost', port=1883):
    ...
    while True:
        ...
        print(f'[PUBLISH][DPIR2] {publish_data_limit} values')
        event.clear()
...
def motion_detected_callback(publish_event, settings):
    global publish_data_counter, publish_data_limit
    print(f"[DPIR2] motion detected {time.strftime('%H:%M:%S', time.localtime())}")
    ...
    with counter_lock:
        dpir_batch.append(('DPIR2', json.dumps(payload), 0, True))
...
def run_dpir2(settings, threads, stop_event):
    if settings.get('simulated', True):
        ...
        print("Starting DPIR2 PIR sensor")
